...
class UIArchiveListViewController: UICollectionViewController {
    ...
    func setupLayout() {
        ...
        if let viewWidth = parent?.view.bounds.width {
            if viewWidth / 3 < 160 {
                ...
                layout.itemSize = CGSize(width: width, height: width / 2 * 3 + 10)
            } else {
    ...
    func setupCell() {
        collectionView.register(UIArchiveCell.self, forCellWithReuseIdentifier: "Archive")
        ...
        let cellRegistration = UICollectionView.CellRegistration<UIArchiveCell, StoreOf<GridFeature>> {
        ...
        dataSource = UICollectionViewDiffableDataSource<Section, StoreOf<GridFeature>>(collectionView: collectionView) { collectionView, indexPath, itemStore in
        ...
        dataSource.supplementaryViewProvider = { [weak self] collectionView, kind, indexPath in
            let footer = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: LoadingReusableView.reuseIdentifier, for: indexPath) as!LoadingReusableView
    ...
    func setupObserve() {
        observe { [weak self] in
            ...
            var snapshot = NSDiffableDataSourceSnapshot<Section, StoreOf<GridFeature>>()
            snapshot.appendSections([.main])
            snapshot.appendItems(Array(store.scope(state: \.archivesToDisplay, action: \.grid)))
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if store.lanraragiUrl.isEmpty == false && store.archives.isEmpty && store.loadOnAppear {
            manualTriggerPullToRefresh()
...
extension UIArchiveListViewController: UICollectionViewDelegateFlowLayout {
        ...
        if indexPath.item == collectionView.numberOfItems(inSection: 0) - 1 {
            if store.loading == false && store.archives.count < store.total {
                Task {
                    ...
                    await store.send(.appendArchives(String(store.archives.count))).finish()
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        guard let selectedItemStore = dataSource.itemIdentifier(for: indexPath) else { return }
        let readerStore = Store(initialState: ArchiveReaderFeature.State.init(archive: selectedItemStore.$archive)) {
            ArchiveReaderFeature()
        }
        navigationController?.pushViewController(UIArchiveReaderController(store: readerStore), animated: true)
    }

    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForFooterInSection section: Int) -> CGSize {
        return isLoading ? CGSize(width: collectionView.bounds.width, height: 80) : .zero
    }
}
